...
class Game {
  ...
  displayEndScreen(){
    const canvasResult = document.getElementById('result-screen');
    canvasResult.width = 200;
    canvasResult.height = 400;
    let ctx = canvasResult.getContext('2d');
    ctx.fillStyle = "white";
    ctx.fillRect(0, 0, canvasResult.width, canvasResult.height);
    ctx.fill();
    ctx.strokeStyle = "black";
    ctx.strokeRect(0, 0, canvasResult.width, canvasResult.height);
    ctx.stroke();
    ctx.fillStyle = "black";
    ctx.font = "18px Impact";
    ctx.textAlign = "center";
    ctx.fillText(this.winner, canvasResult.width/2, canvasResult.height /4);
    ctx.font = "16px Arial";
    ctx.fillText("Press Enter to Restart", canvasResult.width/2, canvasResult.height * 3/4)
  }
